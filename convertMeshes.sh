...
rem @set inputMeshFormat=ply
@set inputMeshFormat=obj


rem Output Mesh File variables
@set outputFolder=C:\multiMeshScripting\input\3D
...
for %%I in (*.%inputMeshFormat%) do (
		if exist %outputFolder%\%%~nI.nxs (
			echo "***********************************************"
			echo "already converted:"
			echo %outputFolder%\%%~nI.nxs
		) else (
			rem convert to ply
			%meshlabserverPath% -i %%I -o %%~nI.%outputMeshFormat% -s texture2color.mlx %outputMeshOptions%
			echo ..\%outputFolder%\%%~nI.ply
			rem convert to nxs
			..\..\Nexus\nxsbuild %outputFolder%\%%~nI.ply -o %outputFolder%\%%~nI_temp.nxs
			rem apply compression
			..\..\Nexus\nxsedit %outputFolder%\%%~nI_temp.nxs -z -o %outputFolder%\%%~nI.nxs
			echo "***********************************************"
			echo "successfully created:"
			echo %outputFolder%\%%~nI.nxs
			rem deleting temporary items
			del %%~nI.ply
			del %%~nI_temp.nxs
		)
		)
...
rem for %%I in (*.%inputMeshFormat%) do (%meshlabserverPath% -i %%I -o ..\%outputFolder%\%%~nI.%outputMeshFormat% -s ..\%mlxScriptFolder%\%mlxScriptFile% %outputMeshOptions%)
rem To get help on the "for" syntax use: for /?
